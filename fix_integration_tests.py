...
"""
    ...
    # metadata 파라미터 제거
    content = re.sub(r"(stock_service\.reserve_stock\([^)]*),\s*metadata=[^,)]+([^)]*\))", r"\1\2", content)
    ...
    content = re.sub(r"stock_service\.reserve_stock\([^)]+\)", add_user_to_reserve_stock, content)
    ...
    def fix_cancel_reservation(match):
        ...
        if "user=" not in call and ", regular_user" not in call and ", superuser" not in call:
            ...
            call = re.sub(r"cancel_reservation\(([^,]+),\s*", r"cancel_reservation(\1, regular_user, ", call)
        return call

    content = re.sub(r"stock_service\.cancel_reservation\([^)]+\)", fix_cancel_reservation, content)
    ...
    def fix_confirm_reservation(match):
        ...
        if "user=" not in call and ", superuser" not in call:
            ...
            call = re.sub(r"confirm_reservation\(([^,)]+)\)", r"confirm_reservation(\1, superuser)", call)
        return call

    content = re.sub(r"stock_service\.confirm_reservation\([^)]+\)", fix_confirm_reservation, content)
    ...
    content = re.sub(r"stock_service\.inbound_stock\([^)]+\)", fix_inbound_stock, content)
