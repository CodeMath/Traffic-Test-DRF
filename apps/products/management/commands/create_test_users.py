...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(f"🚀 Traffic Django 테스트 사용자 생성 도구")
        )
        ...
        self.stdout.write(
            "📝 Locust 테스트 실행: poetry run locust -f locust_test/locustfile.py --host=http://localhost:8000"
        )
    ...
    def create_users(self, count: int, batch_size: int):
        ...
        if existing_count >= count:
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ 이미 {existing_count:,}명의 사용자가 존재합니다. 추가 생성을 건너뜁니다."
                )
            )
            return
            ...
            if batch_users:
                try:
                    with transaction.atomic():
                        ...
                        self.stdout.write(
                            f"⏳ {created_count:,}/{users_to_create:,} 사용자 생성 중... ({progress:.1f}%)"
                        )
                except Exception as e:
                    self.stdout.write(
                        self.style.ERROR(f"❌ 배치 생성 실패: {e}")
                    )
                    continue
        ...
        self.stdout.write(
            self.style.SUCCESS(f"✅ 총 {total_users:,}명의 테스트 사용자가 생성되었습니다!")
        )
    ...
    def verify_users(self, expected_count: int):
        ...
        if total_count >= expected_count:
            ...
            self.stdout.write(
                self.style.WARNING(
                    f"⚠️ 예상 사용자 수({expected_count:,})보다 적습니다({total_count:,})"
                )
            )